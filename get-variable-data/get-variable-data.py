...
base_url = 'https://model.oxfordeconomics.com/api/v1'
access_token = <insert your access token here>
headers = {'Authorization' : f'Bearer {access_token}'}

# Discover the id of a Global Economic Model release forecast
gemFolderResponse = requests.get(f"{base_url}/resources/oxford-economics/releases/GEM", headers=headers)
gemFolder = gemFolderResponse.json()
forecastId = gemFolder["Children"][0]["Id"] # Pick first forecast

# Get all indicators for a forecast
indicatorsResponse = requests.get(f"{base_url}/forecast-contents/{forecastId}/indicators", headers=headers)
indicators = indicatorsResponse.json()

 # Get all groups (locations) for a forecast
groupsResponse = requests.get(f"{base_url}/forecast-contents/{forecastId}/groups", headers=headers)
...
variablesResponse = requests.post(f"{base_url}/forecast-contents/{forecastId}/variables?onlyProps=Values,Metadata", headers=headers, json=variableSelection)
